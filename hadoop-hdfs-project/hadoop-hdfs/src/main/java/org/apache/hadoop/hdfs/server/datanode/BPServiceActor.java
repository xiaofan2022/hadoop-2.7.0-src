...
class BPServiceActor implements Runnable {
  ...
  private void connectToNNAndHandshake() throws IOException {
    ...
    /**
     *
     * HA：hadoop1(active)  hadoop2(standBy)
     *       info    ==       info
     * hadoop3:(actor1 actor2)
     *
     */
  ...
  public void run() {
    ...
    try {
      while (true) {
        ...
        try {
          //TODO 注册核心代码
          //1. 注册的这个代码是比较重要的，千方百计的保证能执行成功。
          //2. datanode  -> namenode进行注册 ，涉及到网络，不稳定因素
      ...
      while (shouldRun()) {
        try {
        	//TODO 发送心跳
          offerService();//读这个代码来验证，今天晚上有没有收获。
        } catch (Exception ex) {
